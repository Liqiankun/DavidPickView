...
#import "DavidPickView.h"
#import "AppDelegate.h"
@interface DavidPickView ()<UIPickerViewDelegate,UIPickerViewDataSource>
{
    NSString *_titleStr;
}
...
//   AppDelegate *appDelegate = (AppDelegate *)[[UIApplication sharedApplication] delegate];
    //CGFloat viewWidth = appDelegate.rootController.view.frame.size.width ;
    CGFloat pickViewWidth = [UIScreen mainScreen].bounds.size.width;
    pickView.frame=CGRectMake(0, ZHToobarHeight, pickViewWidth, pickView.frame.size.height);
...
-(void)setUpDatePickerWithdatePickerMode:(UIDatePickerMode)datePickerMode{
    ...
    CGFloat pickViewWidth = [UIScreen mainScreen].bounds.size.width;
    datePicker.frame=CGRectMake(0, ZHToobarHeight,pickViewWidth,datePicker.frame.size.height);
...
-(UIToolbar *)setToolbarStyle{
    ...
    UIBarButtonItem *titleItem = [[UIBarButtonItem alloc] initWithTitle:_titleStr style:UIBarButtonItemStylePlain target:nil action:nil];
    
    UIBarButtonItem *right=[[UIBarButtonItem alloc] initWithTitle:@"确定" style:UIBarButtonItemStylePlain target:self action:@selector(doneClick)];
    toolbar.items=@[lefttem,centerSpace,titleItem,centerSpace,right];
    return toolbar;
}
...
-(void)remove{
    ...
    if ([self.delegate respondsToSelector:@selector(didClickCancelItem:)]) {
        [self.delegate didClickCancelItem:self];
    }
    if (![_titleStr isEqualToString:@"选择时间"]) {
...
-(void)doneClick
{
    if (_pickerView) {
    ...
    }else if (_datePicker && _datePicker.datePickerMode == UIDatePickerModeDate) {
       ...
       _resultString = [dateFormat stringFromDate:_datePicker.date];
        
    }else if(_datePicker && _datePicker.datePickerMode == UIDatePickerModeTime)
    {
